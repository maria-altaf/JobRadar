"""The dashboard: a FastAPI app serving one server-rendered page plus a JSON API.

Server-rendered because the interesting property of this page is that it keeps
working: no client-side data fetching, no charting CDN, no build step. The JSON
endpoints exist so the same data is machine-readable -- ``/api/health`` in
particular is a plain health endpoint an uptime monitor can poll.

Attribution: Remote OK's API terms require a followed link back to the posting
and a credit naming Remote OK. Both are honoured below; the outbound links
carry ``rel="noopener"`` only, deliberately not ``nofollow``.
"""
...
# Vercel gives the function a read-only filesystem and a pooled Postgres; the
# schema is created by the scraper, so only ensure it when running locally
# against a SQLite file that may not exist yet.
